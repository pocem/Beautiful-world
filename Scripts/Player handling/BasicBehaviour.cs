...
public class BasicBehaviour : MonoBehaviour
{
	...
	void Update()
	{
        ...
        void Attack()
        {
            // Check if any behaviour is currently overriding attack.
            if (!IsOverriding() && !GetTempLockStatus())
            {
                // Call the attack function on the active behaviour.
                foreach (GenericBehaviour behaviour in behaviours)
                {
                    if (behaviour.isActiveAndEnabled && currentBehaviour == behaviour.GetBehaviourCode())
                    {
                        behaviour.AttackAnim();
                        break;
                    }
                }
            }
